...
<nav class="flex mb-8 text-slate-400 text-sm" aria-label="Breadcrumb">
    ...
    <div class="max-w-lg mx-auto bg-brand-panel rounded-xl shadow-2xl border border-slate-700 overflow-hidden">
        ...
        <div class="h-2 bg-gradient-to-r from-yellow-500 to-orange-600 w-full"></div> <div class="p-8">
            <g:form controller="main" action="update" id="${prospecto.id}" method="PUT" name="schoolForm" 
                ...
                <div>
                    ...
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <i id="icon-user" class="fas fa-user-edit text-slate-500 transition-colors"></i>
                        </div>
                        
                        <input type="text" id="nombre" name="nombre" 
                               value="${prospecto?.nombre?.encodeAsHTML()}" 
                               maxlength="50"
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // --- SEGURIDAD: Validación JS (Idéntica a Create) ---
            const inputNombre = document.getElementById('nombre');
            const iconUser = document.getElementById('icon-user');
            const errorNombre = document.getElementById('error-nombre');

            // Disparamos validación inicial por si el dato cargado de BD tiene algo raro (opcional)
            ...
            document.getElementById('schoolForm').addEventListener('submit', function(e) {
                ...
                // RECAPTCHA: Verifica si realmente quieres pedir captcha para EDITAR. 
                // Si decides quitarlo, borra este bloque y el div del HTML.
